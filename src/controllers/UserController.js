...
export default class UserController {
  ...
  static async getAllByUsername(req, res) {
    ...
    return user
      ...
      : res
          ...
          .json({ errors: { user: "no user with this username found" } });
  }
  ...
  static async updateUserRole(req, res) {
    ...
    if (user.role === role) {
      return res
        .status(status.EXIST)
        .send({ message: "The user already has this role" });
    }
    if (!req.body.role) {
      return res
        .status(status.BAD_REQUEST)
        .send({ message: "the role can not be empty" });
    }
    const updatedUser = await User.update({ role }, { username });
    delete updatedUser.password;
    return res
      .status(status.OK)
      .json({ message: "roles updated successfully", updatedUser });
  }
    ...
    if (follow.errors) {
      return follow.errors.name === "SequelizeUniqueConstraintError"
        ? res
            .status(status.EXIST)
            .send({
              errors: { follow: `You are already following "${username}"` }
            })
        : res
            .status(status.SERVER_ERROR)
            .json({ errors: "oops, something went wrong" });
    }
